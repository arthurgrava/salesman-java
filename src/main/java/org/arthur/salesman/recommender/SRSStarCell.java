 ...
 */
public class SRSStarCell implements Runnable {

    private String author;
    private Map<String, List<Citation>> ratings;
    private List<Similar> neighbors;
    private List<Similar> similarities;
    ...
    public SRSStarCell(String author, Map<String, List<Citation>> ratings, List<Similar> neighbors, List<Similar> similarities,
    ...
    private double predict(String item) {
        ...
        for (Similar neighbor : neighbors) {
            ...
            double weight = getNeighborScore(neighbor);

            if (rate != RecommenderUtils.WRONG) {
                num += (weight * (rate - mean));
            }
            denom += weight;
        }
    ...
    private Double getNeighborScore(Similar neighbor) {
        int idx = similarities.indexOf(neighbor);

        if (idx > 0) {
            double score = similarities.get(idx).getScore();
            return score;
        }
