...
import org.arthur.salesman.recommender.SRSStarCell;
...
/**
 * @author Arthur Grava (arthur.grava at gmail.com) - 2016.04.25
 */
public class SRSStarRunner {
    ...
    public SRSStarRunner(String citationsPath, String coauthorsPath, String similaritiesPath, String meansFile,
                         String authors, String target, int topK, int topN) {
    ...
    public void execute(int coreSize, int maxSize) throws Exception {
        ...
        try {
            ...
            Map<String, List<Similar>> similaritiesRelations = FullSimilarityReader.readFile(similaritiesPath);
            ...
            while ((line = br.readLine()) != null) {
                ...
                while (!running) {
                    try {
                        ...
                        tpe.execute(
                                new SRSStarCell(
                                        authorId, citations, similarities.get(authorId), similaritiesRelations.get(authorId),
                                        means, topK, bw, topN
                                )
                        );
                        running = true;
    ...
    public static SRSStarRunner getCalculator(Properties props) throws IOException {
        String citations = props.getProperty("citations.path");
        String similarity = props.getProperty("similarity.path");
        String similaritiesPath = props.getProperty("similarities.path");
        ...
        if (StringUtils.isNoneBlank(citations, similarity, means, target, authors)) {
            System.out.println("Configurations are:\n\t" +
                    Strings.join("\n\t", citations, similarity, means, authors, target, similaritiesPath)
            );

            return new SRSStarRunner(citations, similarity, similaritiesPath, means, authors, target, topK, topN);
        } else {
            System.err.println("To run srs* you need to specify citations.path, similarity.path <coauthors>, similaritiesPath " +
