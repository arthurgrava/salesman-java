...
import org.arthur.salesman.recommender.TSRSCell;
...
public class TSRSRunner {
    ...
    public TSRSRunner(String citationsPath, String coauthorsPath, String meansFile, String authors,
    ...
    public static TSRSRunner getCalculator(Properties props) throws IOException {
        ...
        if (StringUtils.isNoneBlank(citations, similarity, means, target, authors)) {
            System.out.println("Configurations are:\n\t" +
                    Strings.join("\n\t", citations, similarity, means, authors, target)
            );

            return new TSRSRunner(citations, similarity, means, authors, target, topK, topN);
        } else {
